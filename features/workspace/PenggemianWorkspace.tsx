...
import type { Activity, HistoryRecord, Scene, Step, VerificationStatus, View } from "./types";
...
export default function PenggemianWorkspace() {
  ...
  const createCustomActivity = (event: FormEvent) => {
    ...
    if (!name) return;
    const next: Activity = {name,category:customCategory,group:"用户自定义",icon:"新",note:"由你发起 · 碰个面将寻找同需求同学",meta:"自定义人数与预算",featured:false,scene,aliases:[name]};
  ...
  const finishPreferences = () => {
    if (preferenceDraft.length) {
      const nextTags = Array.from(new Set([...personalTags, ...preferenceDraft]));
      setPersonalTags(nextTags);
      setTagDraft(nextTags);
      localStorage.setItem("penggemian-personal-tags", JSON.stringify(nextTags));
    }
    setCategory("推荐");
  ...
  return <main>
    <WorkspaceShell
      ...
      onNavigate={(destination) => {
        const url = new URL(window.location.href);
        if (destination === "workflow") url.searchParams.set("p0", "1");
        else { url.searchParams.delete("p0"); url.searchParams.delete("session"); url.searchParams.delete("p0tab"); }
        window.history.replaceState(null, "", url);
        setView(destination);
      }}
    ...
    >
          ...
          {view === "home" && <HomeView
            cards={homeCards}
            selectedActivity={selectedDiscoveryActivity}
...
{view === "match" && <div className="workspace-view embedded-view match-view">
              ...
              <div className="demo-content">
                ...
                {step===2&&<div className="panel preference-panel">
                  ...
                  {activeScene === "study" && <section className="competency-gate">
                    <div className="competency-head"><div><span>ROLE & EVIDENCE GATE</span><h4>确认你能承担的角色</h4><p>只推荐能力材料已核验、投入时间达标的候选人</p></div><b className={studyGateReady?"ready":""}>{studyGateReady?"可参与匹配":"待完成核验"}</b></div>
                ...
                {step===3 && activity === "数学建模竞赛组队" ? <P0MathModelingPanel startsAtValue={time} weeklyHours={weeklyHours} onBack={()=>setStep(2)} onNotify={notify} onOpenWorkflow={() => { const url = new URL(window.location.href); url.searchParams.set("p0", "1"); url.searchParams.set("p0tab", "sessions"); window.history.replaceState(null, "", url); setView("workflow"); }}/> : step===3&&<InvitationMatch key={`${activity}-${time}-${seats}-${audienceMode}-${JSON.stringify(onlinePreferences)}`} matchPlan={matchPlan} scene={activeScene} activity={activity} time={displayTime} seats={seats} level={level} userLocation={userLocation} venues={venueOptions} selectedVenueId={selectedVenueId} aiServiceFee={AI_SERVICE_FEE} onlinePreferences={onlinePreferences} onSelectVenue={setSelectedVenueId} onFormActivity={completeBooking} onNotify={notify}/>}
          ...
          {view === "partners" && <PartnerView partner={partner} onPartnerChange={setPartner} onNotify={notify}/>}
          ...
          {view === "tags" && <PersonalTagsView groups={personalTagGroups} selectedTags={tagDraft} onToggle={togglePersonalTag} onCancel={() => setView("profile")} onSave={savePersonalTags}/>}
